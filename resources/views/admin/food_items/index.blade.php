        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-4 group">
                ...
                <div>
                    ...
                    <p class="text-xs font-bold text-gray-500 uppercase tracking-widest mt-0.5">Manage Concession Items</p>
                </div>
            </div>
            
            <a href="{{ route('admin.food-items.create') }}" class="relative inline-flex group">
            ...
            <div class="bg-[#111]/80 backdrop-blur-xl border border-gray-800 shadow-2xl rounded-[1.5rem] overflow-hidden">
                ...
                @if($foodItems->isEmpty())
                    <div class="py-24 text-center flex flex-col items-center">
                        ...
                        <h3 class="text-2xl font-black text-white mb-2">No Items Found</h3>
                        <p class="text-gray-500 font-medium max-w-md mx-auto mb-8">You haven't added any specific food or beverage items yet.</p>
                        <a href="{{ route('admin.food-items.create') }}" class="bg-orange-600 hover:bg-orange-500 text-white px-8 py-3.5 rounded-xl font-bold transition shadow-[0_0_15px_rgba(249,115,22,0.3)]">
                            + Create First Item
                        </a>
                    </div>
                @else
                        ...
                        <table class="w-full text-left whitespace-nowrap">
                            <thead class="bg-[#0a0a0a]/50 border-b border-gray-800">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest">Item Info</th>
                                    <th class="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest">Category</th>
                                    <th class="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest">Price</th>
                                ...
                                <tr class="hover:bg-gray-800/30 transition-colors group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="w-16 h-16 rounded-xl overflow-hidden border border-gray-700 bg-[#0a0a0a] shadow-md group-hover:border-orange-500/50 transition-colors relative">
                                            @if($item->imagePath)
                                                <img src="{{ asset('storage/' . $item->imagePath) }}" alt="{{ $item->name }}" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500">
                                            @else
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-base font-black text-white group-hover:text-orange-400 transition-colors">{{ $item->name }}</div>
                                        @if($item->description)
                                            <div class="text-[10px] text-gray-500 font-medium mt-1 truncate max-w-[200px]">{{ $item->description }}</div>
                                        @else
                                            <div class="text-[10px] text-gray-600 italic mt-1">No description</div>
                                        @endif
                                    </td>

                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest bg-orange-500/10 text-orange-400 border border-orange-500/20">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-black text-white">
                                            {{ number_format($item->price) }} <span class="text-[10px] text-gray-500 uppercase">Ks</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if($item->isActive)
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest bg-green-500/10 text-green-500 border border-green-500/20">
                                                <span class="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_5px_#22c55e]"></span> In Stock
                                            </span>
                                        @else
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('admin.food-items.edit', $item->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#0a0a0a] border border-gray-700 hover:border-orange-500 text-gray-400 hover:text-orange-400 hover:bg-orange-500/10 transition-all duration-300" title="Edit Item">
                                            ...
                                            <form action="{{ route('admin.food-items.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#0a0a0a] border border-gray-700 hover:border-red-500 text-gray-400 hover:text-red-500 hover:bg-red-500/10 transition-all duration-300" title="Delete Item">
